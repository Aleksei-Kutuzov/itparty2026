...
from src.db.edu.repo import StaffProfileRepository
from src.db.edu.models import Organization, StaffProfile
...
async def list_pending_users(
    ...
    rows = (
        await db.execute(
            select(User, StaffProfile, Organization)
            .join(StaffProfile, StaffProfile.user_id == User.id)
            .join(Organization, Organization.id == StaffProfile.organization_id)
    ...
    return [
        ...
        for user, profile, organization in rows
    ]
...
async def approve_user_registration(
    ...
    if await StaffProfileRepository(db).get_by_user_id(uid) is None:
        raise HTTPException(
    ...
    return UserResponse.model_validate(verified)
...
async def reject_user_registration(
    ...
    await StaffProfileRepository(db).delete_by_user_id(uid)
